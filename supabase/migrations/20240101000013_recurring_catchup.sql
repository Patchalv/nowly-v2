-- =====================================================
-- FIX: OVERDUE RECURRING TASKS REAPPEAR AFTER COMPLETION
--
-- Problem: the completion trigger advanced a recurring task by exactly
-- ONE period from the template's last scheduled date. When a task was
-- overdue by several periods, the newly generated instance was also in
-- the past. The Today view queries `scheduled_date <= today`, so the
-- fresh uncompleted row reappeared immediately with the same title —
-- the task appeared to vanish and come straight back, and the user had
-- to click once per missed period to catch up.
--
-- Fix: split the per-type date math into a reusable helper and loop it
-- until the next occurrence is strictly after CURRENT_DATE. Missed
-- occurrences are skipped, so one completion always clears the task.
--
-- 'interval_from_completion' is unaffected — it is already defined
-- relative to today rather than to a schedule.
-- =====================================================

-- -----------------------------------------------------
-- Helper: next occurrence after a given date
--
-- Pure date math extracted from generate_next_recurring_instance()
-- (migration 20240101000012) so it can be called repeatedly to skip
-- past occurrences. Reads no tables, so it needs no elevated rights.
-- -----------------------------------------------------
CREATE OR REPLACE FUNCTION public.calculate_next_recurrence(
  template public.recurring_tasks,
  from_date DATE
)
RETURNS DATE
STABLE
...
BEGIN
  ...
  CASE template.recurrence_type
    WHEN 'fixed_daily' THEN
      -- Every N days from the given date (maintains fixed schedule)
      next_date := from_date + COALESCE(template.interval_days, 1);
    ...
    WHEN 'fixed_weekly' THEN
      ...
      LOOP
        ...
        IF template.days_of_week IS NOT NULL AND adjusted_dow = ANY(template.days_of_week) THEN
          ...
          IF week_interval = 1 OR (temp_date - from_date) >= ((week_interval - 1) * 7 + 1) THEN
            next_date := temp_date;
            EXIT;
          END IF;
        END IF;
        ...
        IF temp_date > from_date + 60 THEN
          -- Fallback: just add interval weeks
          next_date := from_date + (week_interval * 7);
          EXIT;
        END IF;
      END LOOP;
    ...
    WHEN 'fixed_monthly' THEN
      ...
      first_of_month := (DATE_TRUNC('month', from_date) + (month_interval || ' months')::INTERVAL)::DATE;
...
-- -----------------------------------------------------
-- Trigger: generate the next instance on completion,
-- skipping any occurrences that are already in the past
-- -----------------------------------------------------
